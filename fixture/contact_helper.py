...
class ContactHelper:
    ...
    def change_field_value(self, fieldname, text):
        wd = self.app.wd
        if text is not None:
            wd.find_element(By.NAME, fieldname).click()
            wd.find_element(By.NAME, fieldname).clear()
            wd.find_element(By.NAME, fieldname).send_keys(text)
